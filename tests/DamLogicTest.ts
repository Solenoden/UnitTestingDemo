    ...
    describe('Remove Forbidden Ducks', function () {
        it('should remove ducks of forbidden species', function () {
           ...
           dam.ducks.forEach(currentDuck => {
               const duckIsForbidden = dam.forbiddenDuckSpecies.findIndex((forbiddenSpecies) => forbiddenSpecies === currentDuck.species) !== -1;
...
const getMockDam = () => {
    return new Dam(
        30,
        [
            new Duck('Margret', 3, DuckSpecies.DOMESTIC),
            new Duck('Quackles', 1, DuckSpecies.MANDARIN)
        ],
        [
            DuckSpecies.MANDARIN,
            DuckSpecies.KING_EIDER
        ]
    )
}
